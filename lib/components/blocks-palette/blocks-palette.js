...
export default Vue.extend({
  ...
  computed: {
    ...mapState('admin', ['blocks']),
    ...mapGetters('admin', { groups: 'blockGroups' }),
  },
  methods: {
    show() {
  ...
  render(h) {
    ...
    if (Array.isArray(this.blocks)) {
      this.blocks.forEach((blockMeta) => {
        ...
        groupedBlocks[groupName].set(generateId(), {
          meta: blockMeta,
          component: h('blocks-palette-item', { props: blockMeta }),
        })
      })
    }
    ...
    return h('div', { class: ['admin-blocks-palette', 'bg-grey-9', 'shadow-5'] }, [
