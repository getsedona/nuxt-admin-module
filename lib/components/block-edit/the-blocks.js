...
export default Vue.extend({
  ...
  created() {
    this.blocks.map((item) => {
      const componentFile = this.$store.getters['admin/blockFileByName'](item.component)
  ...
  render(h) {
    ...
    this.blocks.forEach((block) => {
      const component = h(this.blockComponents.get(block.component), { props: { text: block.props.text } })
