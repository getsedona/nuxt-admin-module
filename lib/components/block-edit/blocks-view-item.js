...
export default Vue.extend({
  ...
  render() {
    ...
    const removeConfirm = <div class={['fit', 'row', 'q-gutter-x-sm', 'justify-center']}>
    ...
    const headerItemIcon = (<q-item-section avatar={true}>
        <q-avatar
    ...
    const headerItemTitle = (<q-item-section>
      <q-item-label>{this.meta?.title || this.component}</q-item-label>
      {this.meta.description ? <q-item-label caption={true} lines={2}>{this.meta.description}</q-item-label> : undefined}
    </q-item-section>)

    // Block item context menu

    const headerItemMenu = <q-item-section side={true}>
