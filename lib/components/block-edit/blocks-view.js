...
export default Vue.extend({
  ...
  methods: {
    ...
    /**
     * Change block params event
     *
     * @param {string} id block id
     * @param {object} params changed params object // {[name:string]:value:any}
     */
    __changeParams(id, params) {
      this.$emit('change-params', { id, params })
    },
  },
  render(h) {
    ...
    const toolbar = h('q-toolbar', { class: 'q-pa-sm' }, [
      ...
      if (this.isBlockExists(block.component)) {
        items.add(
          h('blocks-view-item', {
            ...
            key: block.id,
            on: {
              remove: ({ id }) => this.removeBlock(id),
              'change-params': (params) => this.__changeParams(block.id, params),
            },
          })
        )
      } else {
