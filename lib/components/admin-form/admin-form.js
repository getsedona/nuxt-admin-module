...
export default Vue.extend({
  ...
  created() {
    this.schema.fields.forEach((field) => {
      this.$set(this.form, field.name, get(field, 'value', ''))
    })
  },
  render(h) {
    ...
    const actions = h('div', { class: 'q-gutter-sm' }, [
      ...
      h('q-btn', { props: { label: 'Reset', type: 'reset', flat: true }, class: 'full-width' }),
    ])
    return h('div', { class: 'q-pa-md' }, [
      h(
        'q-form',
        {
          ...
          on: { submit: () => this.$emit('submit'), reset: () => this.$emit('reset') },
        },
        [...inputs, actions]
      ),
    ])
